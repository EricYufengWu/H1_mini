...
#define MID_LEG  300
#define MOVEVAL  20
int MID[] = {320,325,310,325,330,305,340-200,315-200,310,300,330,290};   //"-250" are just for linear servos
...
void reset_pos(){
    ...
    pwm.setPWM(8, 0, MID[8] + 40);     //Switch this if legs are moving in the opposite direction   
    ...
    pwm.setPWM(9, 0, MID[9] + 40);     //Switch this if legs are moving in the opposite direction   
...
void backward(int dur){
    ...
    for (int i = 0; i < dur; i++){
        int legmove = -20;       //amount that the leg moves up and down
        for (uint16_t pulselen = BASEMIN; pulselen < MID_ALL; pulselen++) {
            ...
            legmove--;      //Switch this if legs are moving in the opposite direction      
        }
